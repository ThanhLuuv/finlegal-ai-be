// Pure Reciprocal Rank Fusion (RRF) Reranker
// Merges Dense Vector Search and Lexical Keyword Density natively without hardcoded regex patterns
...
export class LexicalReranker {
   ...
   */
  public rerank(matches: RawRetrievedMatch[], keywords: string[], topK = 8): EvidenceBlock[] {
    ...
    // 2. Lexical Rank Map (sorted by query keyword density)
    const lexicalScored = matches.map(match => {
      ...
      const lexicalRatio = matchedCount / totalKeywords;
    ...
    // 3. Calculate Pure Reciprocal Rank Fusion (RRF) Scores
    const reranked: EvidenceBlock[] = matches.map(match => {
      ...
      const itemScored = lexicalScored.find(ls => ls.match.chunkId === match.chunkId);
      ...
      return {
        ...
        lexicalScore: itemScored?.lexicalRatio || 0,
        rrfScore,
        citation: {
          documentId: match.metadata.docId,
